  ...
  return (
    <div className="relative w-full mt-8 sm:mt-0">
      ...
      <div className="mb-3 sm:mb-4">
        <button 
          onClick={() => router.back()} 
          className="flex items-center gap-2 px-4 py-2.5 sm:py-3 bg-black/95 backdrop-blur-md rounded-lg sm:rounded-xl border-2 border-white/20 sm:border text-white shadow-2xl min-h-[48px] sm:min-h-0"
          aria-label="Go back"
        >
          <ArrowLeft size={20} className="sm:w-5 sm:h-5 flex-shrink-0" />
          ...
          {streams.length > 1 && (
            <div className="relative flex-shrink-0">
              <select 
                ...
                className="bg-transparent text-white text-[9px] xs:text-[10px] font-black border-none rounded-full py-1 pl-1.5 xs:pl-2 pr-5 xs:pr-6 focus:outline-none appearance-none cursor-pointer uppercase tracking-tight touch-manipulation min-h-[32px]"
              ...
              <ChevronDown size={10} className="xs:w-3 xs:h-3 absolute right-1 xs:right-1.5 top-1/2 -translate-y-1/2 text-miraj-gold pointer-events-none" />
            </div>
          )}
      ...
      <div 
        ref={containerRef} 
        className={`relative bg-black w-full overflow-hidden ${isFullscreen ? 'fixed inset-0 z-[100]' : 'aspect-video rounded-xl border border-white/10 shadow-2xl'}`}
      >
        {/* Fullscreen Button - ALWAYS VISIBLE, NO HOVER EFFECTS */}
        <div className="absolute top-3 sm:top-4 left-1/2 -translate-x-1/2 z-50 pointer-events-auto">
          <button 
            onClick={toggleFullscreen} 
            className="p-2 sm:p-2.5 bg-black/80 backdrop-blur-md rounded-full border-2 border-white/40 text-white shadow-2xl min-h-[44px] min-w-[44px] sm:min-h-[48px] sm:min-w-[48px] flex items-center justify-center"
